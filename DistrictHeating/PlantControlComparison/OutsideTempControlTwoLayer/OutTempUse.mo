...
model OutTempUse
  ...
  DistrictHeating.Components.Storage.StorageTwoLayer
                          storageTwoLayerNoAssert(
    V=137,
    H=22,
    rho=water.rho,
    cp=water.cp,
...
connect(consumerTimeDependExt.positive_heat_flow, Net_source.y[6])
  ...
  connect(consumerTimeDependExt.heat_flow, storageTwoLayerNoAssert.port_b)
    annotation (Line(points={{40,0},{26,0},{10,0}}, color={191,0,0}));
  connect(Outside_Temp_source.y[6], storageTwoLayerNoAssert.u) annotation (Line(
        points={{7.21645e-016,-39},{0,-39},{0,-11.4}}, color={0,0,127}));
  connect(storageTwoLayerNoAssert.y, outsideTempControlContinous.u1)
    annotation (Line(points={{0,10.6},{0,29.6}},          color={0,0,127}));
  connect(storageTwoLayerNoAssert.port_a, boilerInFinite.thermal_heat_flow)
    annotation (Line(points={{-10,0},{-26,0},{-40,0}}, color={191,0,0}));
  connect(outsideTempControlContinous.u, storageTwoLayerNoAssert.u) annotation (
     Line(points={{11.2,40},{20,40},{20,-20},{0,-20},{0,-11.4}}, color={0,0,127}));
  connect(outsideTempControlContinous.y, boilerInFinite.nominal_heat)
