...
<p><span style=\"font-family: Arial;\">It doesn&apos;t matter, whether the controller input unit is temperature or heat, as long as the parameter Target_Store_Fill or the extern target have the same unit (SIunits required). Example: If the store has unit Kelvin, the target also needs unit Kelvin as parameter.  </span></p>
...
<p><img src=\"modelica://DistrictHeating/Resources/Images/PID_Table.png\"/></p>
<p><span style=\"font-family: Arial,sans-serif;\">In many cases, the most useful initial condition is <b>SteadyState</b> because initial transients are then no longer present. If initType = InitPID.SteadyState, then in some cases difficulties might occur. The reason is the equation of the integrator: </span></p>
